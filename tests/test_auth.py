...
class TestAsyncs(unittest.IsolatedAsyncioTestCase):
    async def test_get_current_user(self):
        ...
        wrong_composed_token = jwt.encode({'dora': 'dura'}, key=config.SECRET_KEY, algorithm=config.ALGORITHM)
        ...
        auth.controller.find_user.return_value = test_user
        good_token = jwt.encode({'username': 'mmmity'}, key=config.SECRET_KEY, algorithm=config.ALGORITHM)
