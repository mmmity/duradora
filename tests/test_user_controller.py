...
class TestUserController(unittest.TestCase):
    ...
    @patch('sqlite3.connect')
    def test_create(self, mock_connect: MagicMock):
        controller = UserController('duradora.db')
        controller.create_user(User(username='mmmity', password='cringe', is_admin=True))
        controller.cur.execute.assert_called_once_with('INSERT INTO users VALUES(?, ?, ?)', ('mmmity', 'cringe', True))
        controller.con.commit.assert_called_once()

    @patch('sqlite3.connect')
    def test_user_from_tuple(self, mock_connect: MagicMock):
        controller = UserController('duradora.db')
    ...
    @patch('sqlite3.connect')
    def test_find_user(self, mock_connect: MagicMock, mock_connection: MagicMock, mock_cursor: MagicMock):
    ...
    @patch('sqlite3.connect')
    def test_update_user(self, mock_connect: MagicMock):
        controller = UserController('duradora.db')
        controller.update_user(User(username='mmmity', password='flex', is_admin=False))
        controller.cur.execute.assert_called_once_with('UPDATE users SET password = ?, is_admin = ? WHERE username = ?', ('flex', False, 'mmmity'))
